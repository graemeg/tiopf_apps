...
begin
  TV.Data := nil ;
  FData.Clear ;
  gTIOPFManager.VisMgr.Execute( cgsDeployMgrReadPK, FData ) ;
...
begin
  ...
  gTIOPFManager.VisMgr.Execute(     cgsDeployMgrSave, FData ) ;
...
procedure TFormMain.TVSelectNode(ptiTreeView: TtiTreeView;
  ...
  pParentData: TObject);
begin
  gTIOPFManager.VisMgr.Execute( cgsDeployMgrReadAppDetails, TtiObject( pData )) ;
...
begin
  ...
  begin
    gTIOPFManager.VisMgr.Execute( cgsDeployMgrReadAppDetails, FData ) ;
    ReadDeployFromDirty( FData ) ;
    Save ;
    Close ;
  end ;
